...
X = data[:, :2]
Y = np.array([[normalize(i)] for i in data[:, 2]])
...
for row in data:
    ...
    print(
        ...
        f"{result:7.3f} ({cls:2.0f}), "
        f"ожидаем {row[2]:2.0f}"
    )
...
for row in data:
    ...
    print(
        f"({row[0]:2.0f},{row[1]:2.0f}) | "
        f"{value:7.3f} ({get_class(value):2.0f}), "
        f"ожидаем {row[2]:2.0f}" )

print("\nРежим ввода (выход: letmeleavepls)")
